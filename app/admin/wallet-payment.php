...
if(isset($_POST['checkpaymentstatus'])){
    echo $payments->checkWalletPayment();
}
if(empty($payment)){
  include('404.php');
  die();
            ...
            <div class="row invoice-info">
                ...
                <div class="col-sm-4 invoice-col">
                    ...
                    <address>
                        ...
                        <strong class="text-capitalize"><?php echo $payment['type'] ?> <?php echo $payment['method'] ? ' - '.$payment['method'] : '' ?></strong><br>
                        <small>Status: </small>
                        <?php
                        $payment['status'];
                        $status=array(
                        ...
                        echo '</small>';
                        ?>
                        <h3 class="text-capitalize"><small>Amount: </small> $<?php echo number_format($payment['amount'],2); ?></h3>
                   </address>
                </div>
            </div>
            <hr/>
                    ...
                    <div class="row">
                        ...
                        */
                        ...
                        <div class="col-md-4 col-md-offset-4">
                            ...
                            <input type="tex" name="amount" class="form-control" required value="<?php echo $payment['net_amount'] ? $payment['net_amount'] : 0 ?>">
                        </div>
                        <!-- <div class="col-md-3">
                        ...
                        <div class="col-md-12 text-center mt-1">
                            <button type="button" onclick="confirmPayment(2)" name="approved" class="btn btn-success"><?php echo $payment['status']==2 ? 'Change Payment' : 'Approve Payment' ?></button>
                            <button type="button" onclick="confirmPayment(3)" name="decline" class="btn btn-danger">Decline</button>
                            <a href="<?php echo baseurl.'/admin/wallet.php' ?>" class="btn btn-default">Back</a>
...
function confirmPayment(arg){
    ...
    if(arg==2){
        <?php if($payment['status']==2) : ?>
            approve=confirm('Are you sure to change this payment for user wallet ?');
        <?php else : ?>
            approve=confirm('Are you sure to approve this payment for user wallet ?');
        <?php endif ?>
    }
    if(approve==true){
        $('#status').val(2)
    ...
    if(arg==3){
        decline=confirm('Are you sure to decline this payment for user wallet ?');
    }
    if(decline==true){
        $('#status').val(3)
        $('form').trigger('submit');
