...
$allcamps=$campaign->getAllReversionCampaign($_REQUEST['token']);
$rid=NULL;
if($_REQUEST['reversion']){
    $rid=$_REQUEST['reversion'];
}
$camp=$campaign->reversionCampaign($_REQUEST['token'],$rid);
if(empty($camp)){
  include('404.php');
  die();
}
            ...
            <div class="col-xs-12">
                <div class="row text-center">
                    ...
                    if(!empty($allcamps)){
                        $i=1;
                    ?>
                    <ul class="reversion-cover">
                    <?php
                        foreach ($allcamps as $key => $scamp) {
                            $active='';
                            if(md5($scamp['id'])==$_REQUEST['reversion']){
                                $active='active';
                            }
                    ?>
                        <li class="<?php echo $active ?>">
                            <a class="reversion-action" href="campaign-history.php?token=<?php echo $_REQUEST['token'] ?>&reversion=<?php echo md5($scamp['id']) ?>" data-toggle="tooltip" title="<?php echo date('jS M, Y h:i A',strtotime($scamp['deleted_at'])) ?>"></a>
                        </li>
                    <?php
                        }
                    ?>
          ...
          <div class="row">
            <div class="col-xs-12 table-responsive">
              <table class="table table-striped">
                ...
                <tbody>
                <?php
                $bidcampaigns=$campaign->getAllReversionCampaignsByGroupId($camp['id']);
